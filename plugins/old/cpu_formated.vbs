...
if WScript.Arguments.Count <> 0 then
	...
	if WScript.Arguments.Item(0) = "config" then
		...
		Wscript.Echo "system.type GAUGE"
	...
	For Each objItem in colItems
		cpucount=cpucount+1
		'To make the statistic look nice this could be the solution:
		val_system = val_system + ( Int(objItem.PercentPrivilegedTime)  )
		val_user = val_user + ( Int(objItem.PercentProcessorTime) - Int(objItem.PercentInterruptTime) -  Int(objItem.PercentPrivilegedTime) )
		val_idle = val_idle + (100 - Int(objItem.PercentProcessorTime))
		val_irq = val_irq + (  Int(objItem.PercentInterruptTime)  )
	next
